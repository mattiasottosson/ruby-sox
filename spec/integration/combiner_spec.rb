...
describe "Sox::Combiner" do
  ...
  describe 'concatenation' do
    ...
    it 'should concatenate should write output with 2 channels' do
      ...
      combiner = Sox::Combiner.new(files, :combine => :concatenate, :channels => 2 , :rate => 44100, :normalize => false)
    ...
    it 'should concatenate' do
      ...
      combiner = Sox::Combiner.new(files, :combine => :concatenate, :normalize => false)
      combiner.write(output_file)
      ...
      output_file.should have_same_content_as output_fixture('d_g2_g3_concatenated_c1_r22050.flac')
    end
  end
  ...
  describe 'mixing' do
    ...
    it 'should mix' do
      files = [drums_input, guitar2_input, guitar3_input]
      combiner = Sox::Combiner.new(files, :combine => :mix, :normalize => false)
      combiner.write(output_file)

      File.exists?(output_file).should be_true
      output_file.should have_same_content_as output_fixture('d_g2_g3_mixed_c1_r22050.mp3')
    end

    context 'with normalization' do
      it 'should mix and normalize' do
        files = [drums_input, guitar2_input, guitar3_input]
        combiner = Sox::Combiner.new(files, :combine => :mix, :normalize => true)
        combiner.write(output_file)

        File.exists?(output_file).should be_true
        output_file.should have_same_content_as output_fixture('d_g2_g3_mixed_c1_r22050_norm.mp3')
      end
    end
  end
  ...
  describe 'perfomance' do
    ...
    it 'concatenate' do
      ...
      Benchmark.bm(20) do |x|
        rates.each do |rate|
          ...
          x.report("#{duration}s*#{count}=#{sum_duration}s rate=#{rate}" ) do
            Sox::Combiner.new(small_input_files, :combine => :concatenate, :rate => rate, :normalize => false).write(output_file)
          end
        end
      end
      ...
      rates.each do |rate|
        ...
        Sox::Combiner.new(input_files, :combine => :concatenate, :rate => rate, :normalize => false).write(file)
      ...
      Benchmark.bm(20) do |x|
        ...
        rates.each do |rate|
          x.report("#{duration}s*#{count}=#{sum_duration}s rate=#{rate}" ) do
            Sox::Combiner.new(big_input_files, :combine => :concatenate, :rate => rate, :normalize => false).write(output_file)
          end
        end
      end
    end
